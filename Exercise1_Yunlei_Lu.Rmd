...
#### The destination frequency of flights that departed from Ausitn 
```{r freq Origin, echo=TRUE}
...
#### The origin frequency of flights that landed at Ausitn
...
flights_df = flights_df %>% arrange(-freq)
flights_df2 = flights_df2 %>% arrange(-freq)

g1 <- flights_df %>% ggplot(aes(x=reorder(City,freq), y=freq)) +
  geom_bar(aes(fill=flights_df$freq), stat='identity') +
  coord_flip() +
  scale_fill_gradientn(name='',colors=colorRampPalette(c("gray","#46ACC8"))(10)) +
  theme(legend.position='none', plot.title=element_text(size=10)) + 
  ggtitle('Number of flights,\nDeparted from Ausitn, 2008') +
  ylab("Number of flights") + xlab("Destination") +
  theme(axis.text=element_text(size=5, face="bold"),
        axis.title=element_text(size=9))
# g1
g2 = ggplot(flights_df2, aes(x=reorder(City, freq), y=freq)) +
  geom_bar(aes(fill=freq), stat='identity') +
  coord_flip() +
  scale_fill_gradientn(name='',colors=colorRampPalette(c("gray","#DD8D29"))(10)) +
  theme(legend.position='none', plot.title=element_text(size=10)) + 
  ggtitle('Number of flights,\nLanded at Ausitn, 2008') +
  ylab("Number of flights") + xlab("Origin") +
  theme(axis.text=element_text(size=5, face="bold"),
        axis.title=element_text(size=9))
# g2
...
USMap + 
  geom_point(data=flights_df, aes(x=long,y=lat,size=freq/5000,color="Departure"),alpha=0.6) + 
  geom_point(data=flights_df2,aes(x=long,y=lat,size=freq/5000,color="Landing"),alpha=0.4) + 
  geom_label_repel(data=flights_df[1:11,1:ncol(flights_df)], aes(x=long,y=lat,label=flights_df$City[1:11]),
                   size=2.5, color = 'black',
                   box.padding = unit(0.1, "lines"),
                   point.padding = unit(0.3, "lines"),
                   segment.color = 'black', alpha=1, force=10) +
  scale_colour_manual(name="",values=c(Departure="red",Landing="#00A5FF")) + 
  scale_size(range=c(0,8)) + 
  theme(legend.title=element_blank()) + 
  coord_map("albers", lat0=39, lat1=50) + 
...
The mean market rent of the buildings in cluster was \$25 per square foot per year, while the mean market rent in the green buildings was \$27.60 per square foot per year: about \$2.60 more per square foot. Because our building would be 250,000 square feet, this would translate into an additional \$250000 x 2.6 = $650000 of extra revenue per year if we build the green building.
Our expected baseline construction costs are \$100 million, with a 5% expected premium for green certification. Thus we should expect to spend an extra \$5 million on the green building.Based on the extra revenue we would make, we would recuperate these costs in $5000000/650000 = 24.7 years, which is much larger than the previous 8 years estimation.
...
ggplot(data=ds_green, aes(x=age, y=Rent_total))+
  geom_point() +
  xlim(0, 40) + ylim(0,40) +
  xlab("Age of Building") + ylab("Rent ($/square feet)") +
